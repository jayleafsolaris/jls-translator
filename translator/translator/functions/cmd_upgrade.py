...
def cmd_upgrade(enforce=False):
    ...
    try:
        ...
        with zipfile.ZipFile(io.BytesIO(response.content)) as z:
            ...
            # The downloaded source may be compiled (see common/code_obfuscate.py
            # and functions/cmd_push.py) -- decompile it in place, in the
            # temp extraction, before anything from it ever touches the
            # real install. If anything can't be decompiled (no matching
            # key cached on this machine, or corrupted content), abort
            # outright rather than install code that can't actually run --
            # nothing has been backed up or touched yet at this point, so
            # there's nothing to restore.
            # The downloaded source may be compiled (see common/code_obfuscate.py
            # and functions/cmd_push.py) -- decompile it in place, in the
            # temp extraction, before anything from it ever touches the
            # real install. The key comes from THIS downloaded cli.py
            # itself (never compiled, carries the key in the clear) so
            # this is correct even right after a key rotation, not from
            # whatever key this machine's own currently-installed cli.py
            # happens to carry. If anything can't be decompiled (missing
            # key line in the downloaded cli.py, or corrupted content),
            # abort outright rather than install code that can't actually
            # run -- nothing has been backed up or touched yet at this
            # point, so there's nothing to restore.
