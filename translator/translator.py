...
def translate_many(google_code, texts, max_workers, progress_cb=None):
    ...
    # Splitting by item count too (bounded by worker count) guarantees
    # multiple batches -- and therefore multiple incremental progress
    # updates -- even when the total text is small.
    target_batch_count = min(len(valid_indices), max(max_workers, 1))
